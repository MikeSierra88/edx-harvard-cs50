...
function Brick:render()
    if self.inPlay then
        ...
        if self.isLocked then
            quadToDraw = gFrames['bricks'][22]
        else
            quadToDraw = gFrames['bricks'][1 + ((self.color - 1) * 4) + self.tier]
        end
        love.graphics.draw(gTextures['main'], quadToDraw, self.x, self.y)
