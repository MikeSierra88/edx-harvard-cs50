...
function StartState:update(dt)
    ...
    if self.mouseX > 210 and self.mouseX < 300 and self.mouseY > 164 and self.mouseY < 184 then
        self.mouseOverMenu = true
        if self.currentMenuItem ~= 1 then
            gSounds['select']:play()
            self.currentMenuItem = 1
        end
    elseif self.mouseX > 210 and self.mouseX < 300 and self.mouseY > 184 and self.mouseY < 208 then
        self.mouseOverMenu = true
    ...
    if not self.pauseInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') or (love.mouse.wasClicked() and self.mouseOverMenu) then
            if self.currentMenuItem == 1 then
